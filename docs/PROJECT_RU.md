...
предыдущий логический шаг, не сбрасывая весь сценарий. Кнопка `⌂ Главная`
доступна на вложенных экранах. Во время пошагового ввода она сохраняет текущий
экран и показывает на главной панели кнопку продолжения черновика.
...
Все календарные даты в Telegram и Google Sheets показываются в формате
...
Строки со статусом `Оплачено`, добавленные пользователем непосредственно в
`Money`, импортируются обратно в служебный журнал при открытии главной панели,
счетов, сводки или последних операций. Перед импортом бот записывает в note
скрытый `tx_id`, поэтому повторный запуск не создаёт дубль. Последующие правки и
физическое удаление импортированной строки отражаются при следующей проверке.

Счёт определяется по `Вид оплаты`: `Наличные`, `Карта RU` или `Карта CRY`.
Сопоставление должно быть однозначным и в строке должна присутствовать сумма в
валюте выбранного счёта. Если подходящих счетов несколько либо не хватает даты
или суммы, строка не импортируется, а бот показывает предупреждение.
...
| `base_currency` | Валюта страны для сводки, например `EUR`, `AED` или `KZT`. |
| `base_currency_rub_rate` | Стоимость единицы текущей базовой валюты в RUB. |
| `currency_rates_json` | Курсы произвольных ISO-валют к RUB для счетов и сводок. |
...
Служебные листы `Траты`, `Счета`, `Категории`, `Настройки` и `Обзор` удалять
нельзя. `Money` остаётся пользовательским представлением и местом ручного ввода,
но не содержит пополнения, переводы, внутренние ID, связи со счетами, историю
удаления и настройки. Служебные листы скрыты и используются как база данных
бота; удалённый лист будет создан заново при следующей инициализации.
...
На экране базовой валюты быстрыми кнопками остаются USD и EUR. Кнопка
`Другая валюта` принимает любой поддерживаемый ISO-код (`GBP`, `AED`, `KZT` и
т. д.), а также частые русские названия и символы: `евро`, `тенге`, `дирхам`,
`$`, `€`, `£`. Распознавание детерминированное и не вызывает OpenAI API. При
первом использовании новой валюты бот просит указать, сколько RUB стоит одна её
единица, и сохраняет курс в `currency_rates_json`.

Для произвольной базовой валюты Telegram-сводка переводит сохранённую
историческую сумму в RUB по текущему настроенному курсу базовой валюты. Поэтому
изменение этого курса меняет представление прежних расходов в базовой валюте,
но не меняет сами строки операций и их сохранённые RUB/USD/JPY-эквиваленты.
...
```json
{
  ...
  "chats": {
    "<chat_id>": {
      ...
      "digestLastSent": { "<spreadsheet_id>": "14.07.2026" }
    }
  }
}
```
...
временный файл с правами `0600` и `rename`, а операции записи сериализованы
самовосстанавливающейся после файловой ошибки очередью. Перед заменой основного
файла сохраняется `state.json.bak`; повреждённый JSON автоматически изолируется
как `state.json.corrupt-*`, после чего состояние восстанавливается из backup.
...
| `ALLOWED_TELEGRAM_USER_IDS` | Да по умолчанию | Numeric user ID через запятую. |
| `ALLOW_PUBLIC_ACCESS` | Нет | Явный opt-in публичного режима; по умолчанию `false`. |
...
Бот работает по принципу fail closed: без allowlist он не запускается. Публичный
режим возможен только с `ALLOW_PUBLIC_ACCESS=true` и не рекомендуется для бота с
правом записи в финансовые таблицы.
...
- fail-closed конфигурация доступа и восстановление очереди JSON-хранилища;
- идемпотентный append журнала при повторе и потерянном ответе Google API;
- восстановление `state.json` из последней валидной резервной копии;
...
Команда `/status` выполняет безопасную read-only проверку активной таблицы и
показывает количество счетов и операций, очередь `pending`/`failed`, время
последней успешной синхронизации `Money` и задержку ответа. Spreadsheet ID и
секреты в этот экран не выводятся.
